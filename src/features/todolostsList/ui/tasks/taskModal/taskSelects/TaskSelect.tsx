...
type ExtendedOption = SelectOptionDefinition<string> & {
	color: string
}
...
function CustomOption(props: OptionProps) {
	const { children, value, className, color, disabled = false } = props;
		...
		<Root>
			...
			>
				<SelectProvider value={contextValue}>
					{options.map((option) => {
						return (
							<CustomOption key={option.value} value={option.value} color={option.color}>
								{option.label}
							</CustomOption>
						);
					})}
