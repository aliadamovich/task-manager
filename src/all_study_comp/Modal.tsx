	...
	return (
		<div>
			...
			<Cards>
				{
					modalData.map(m => {
						return <ModalItem key={m.id}>
										<h3>{m.title}</h3>
										<p>{m.content}</p>
										<Button title='Open' callBack={() => {onOpenClick(m.id)}}/>
...
const Popup = styled.div<PopupType>`
	...
	${props => props.isOpen && css<PopupType>`
		display: flex;
	`}
`
