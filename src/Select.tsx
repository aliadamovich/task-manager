...
export const Select = ({items, initialValue, onChange}: SelectPropsType) => {
	...
	return (
		...
		<div className={s.select} >
			<div className={s.select__default} onKeyDown={onKeyUp} onClick={onChooseClick} tabIndex={0}>City:{selectedItem && selectedItem.title} </div>
			<div className={visibility ? `${s.select__box} ${s.visible}` : s.select__box}>
				{items.map(i => <span key={i.value}
													className={hoveredItem === i ? s.active + ' ' + s.box__item : s.box__item}
													onMouseEnter={() => { onHover(i.value) }} 
													onClick={() => { onOptionClick(i.value) }}>{i.title}
												</span> )}
			</div>
		</div>
